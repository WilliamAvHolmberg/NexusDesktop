...
public final class AccountLauncher {
	...
	public static String getRSPeerJar(){
		...
		String path = "-Xmx384m -XX:+UseParNewGC -XX:+UseConcMarkSweepGC -Xss2m -Dsun.java2d.noddraw=true -Xincgc " + 
				Paths.get(fw.getDefaultDirectory().toString(), "RSPeer", "cache", "rspeer.jar").toString().replace("%20",
		                " ");
		
		//quickfix for my home desktop which contains space in name.
		if(path.contains(" ")) {
			return "./rspeer.jar";
		}
		return path;
